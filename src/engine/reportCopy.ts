...
/** Strip database identifiers and timestamps out of anything a reader will see. */
...
function reportTotals(report: ReportSummary) {
  return report.skillBreakdown.reduce(
    (result, skill) => ({ correct: result.correct + skill.correct, total: result.total + skill.total }),
    { correct: 0, total: 0 },
  )
}

export function friendlyReportTitle(report: ReportSummary) {
  if (report.type === 'comprehensive') return 'Your complete LNAT learning report'
...
export function friendlyReportSummary(report: ReportSummary) {
  ...
  const missText = misses.length === 1
    ...
    : misses.length > 1
      ? ` The review found ${misses.length} skills worth revisiting.`
      : ' No errors were recorded in this set.'
  return `${result}${missText}${next ? ` Next: ${readerText(next.action)}` : ''}`
}
